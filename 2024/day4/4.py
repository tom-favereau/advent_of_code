...
print(count(lines)+count(cols)+count(d))
print(sum((i+2 < len(lines) and j+2 < len(lines[0]) and lines[i][j] + lines[i+1][j+1] + lines[i+2][j+2] in {"MAS", "SAM"} and lines[i][j+2] + lines[i+1][j+1] + lines[i+2][j] in {"MAS", "SAM"}) for i in range(len(lines)) for j in range(len(lines[0]))))
